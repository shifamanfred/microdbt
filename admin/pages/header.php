...
if (!isset($login)) {
  ...
  $query = 'SELECT first_name, last_name, email, username ';
  ...
  if (!$result) {
    ...
    if (isset($page) && $page == 'index') {
      // Obtain number of registered partners
      $query = 'SELECT COUNT(*) AS partner_count FROM partners;';
      $row = mysqli_fetch_assoc(mysqli_query($connect, $query));
      $partner_count = $row['partner_count'];

      // Obtain number of registered borrowers
      $query = 'SELECT COUNT(*) AS borrower_count FROM borrowers;';
      $row = mysqli_fetch_assoc(mysqli_query($connect, $query));
      $borrower_count = $row['borrower_count'];

      // Obtain total number of registered loan users
      $query = 'SELECT COUNT(*) AS loan_users_count FROM credentials ';
      $query .= 'WHERE id NOT IN (SELECT cred_id FROM super_users);';
      $row = mysqli_fetch_assoc(mysqli_query($connect, $query));
      $total_loan_users = $row['loan_users_count'];
    }
  }
}

if (isset($sub_page) && $sub_page == 'partner_profile') {
  if (!isset($_GET['id'])) {
    header('Location: view_partners.php');
    exit;
  } else {
    $id = $_GET['id'];
  }
