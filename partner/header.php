...
if (! (isset($_SESSION['id']) && isset($_SESSION['cred_id']) && isset($_SESSION['role']))) {
  ...
  switch ($table_name) {
    case 'partners':
    $query = 'SELECT business_name, owner_name, owner_last_name, owner_email, username ';
    $query .= 'FROM ' . $table_name . ' t JOIN credentials c ON (cred_id = c.id) ';
    $query .= 'WHERE t.id = ' . $user_id;
    break;

    default:
    $query = 'SELECT first_name, last_name, name, owner_last_name, t.email AS email, username ';
    $query .= 'FROM ' . $table_name . ' t JOIN credentials c ON (cred_id = c.id) ';
